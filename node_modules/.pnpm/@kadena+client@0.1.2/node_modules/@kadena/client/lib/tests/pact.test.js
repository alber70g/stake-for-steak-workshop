...
describe('Pact proxy', () => {
    ...
    it('generates two different commands when generated asynchronously', () => {
        ...
        const pact = pact_1.Pact;
        const tx = pact.modules.coin.transfer('alice', 'bob', 100).createCommand();
        const tx2 = pact.modules.coin['transfer-xchain']('k:1', 'k:1', 'chain1', 'chain2', 100).createCommand();
        expect(getCode(tx2)).toBe('(coin.transfer-xchain "k:1" "k:1" "chain1" "chain2" 100.0)');
        expect(getCode(tx)).toBe('(coin.transfer "alice" "bob" 100.0)');
    });
    it('generates a command with data', () => {
        const sender = 'k:554754f48b16df24b552f6832dda090642ed9658559fef9f3ee1bb4637ea7c94';
        const receiver = 'k:e34b62cb48526f89e419dae4e918996d66582b5951361c98ee387665a94b7ad8';
        const amount = 0.23;
        const signerPubKey = sender.split('k:')[1];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const pact = pact_1.Pact;
        const transaction = pact.modules.coin['transfer-create'](sender, receiver, () => "(read-keyset 'ks)", amount)
    ...
    it('makes a well formatted /local call', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const pact = pact_1.Pact;
        const builder = pact.modules.coin.transfer('from', 'to', 1.234);
        await builder.local('fake-api-host.local.co');
        const body = builder.createCommand();
        expect(cross_fetch_1.default).toBeCalledWith('fake-api-host.local.co/api/v1/local', {
    ...
    it('makes a well formatted /send call', async () => {
        ...
        expect(cross_fetch_1.default).toBeCalledWith('fake-api-host.local.co/api/v1/send', {
            body: JSON.stringify(body),
            headers: { 'Content-Type': 'application/json' },
            method: 'POST',
        });
    ...
    it('makes a well formatted /poll call', async () => {
        ...
        expect(cross_fetch_1.default).toBeCalledWith('fake-api-host.local.co/api/v1/poll', {
...
describe('TransactionCommand', () => {
    ...
    beforeEach(() => {
        transactionCommand = pact.modules.coin['transfer-create'](sender, receiver, () => "(read-keyset 'ks)", amount)
    ...
    it('resets `cmd` `hash` and `signatures` when `addCap` is called', async () => {
        const transaction = transactionCommand.createCommand();
        const updatedTransactionCommand = transactionCommand.addCap('coin.GAS', senderPubKey);
        expect(updatedTransactionCommand.sigs).toEqual([undefined]);
        const updatedTransaction = updatedTransactionCommand.createCommand();
        expect(transaction.cmd).not.toEqual(updatedTransaction.cmd);
        expect(transaction.hash).not.toEqual(updatedTransaction.hash);
